...
def _aggregate_budget_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped: dict[tuple[str, float], list[float]] = {}
    for row in rows:
        quality = row.get("quality_at_budget")
        if quality is None:
            continue
        key = (str(row["variant"]), float(row["time_budget_sec"]))
        grouped.setdefault(key, []).append(float(quality))

    out: list[dict[str, Any]] = []
    for (variant, time_budget_sec), values in sorted(grouped.items(), key=lambda item: (item[0][0], item[0][1])):
        out.append(
            {
                "variant": variant,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Compare QA block sizes (subset_size) with wall-clock sampling and export "
            "run, timeline, and budget summaries."
        )
    )
    ...
    parser.add_argument("--subset-sizes", type=_parse_int_csv, default=[6, 12, 24])
    ...
    parser.add_argument("--experiment-name", default="qa-block-size-comparison")
    parser.add_argument("--run-name-prefix", default="qabs")
    ...
    parser.add_argument("--output-prefix", default="block_size_comparison")
    ...
    for qa_mode in args.qa_samplers:
        for subset_size in args.subset_sizes:
            ...
            for seed in args.seeds:
                ...
                variant = f"qa:{qa_mode}:subset{int(subset_size)}"
                run_name = (
                    f"{args.run_name_prefix}-{args.model}-{args.dataset}-"
                    f"{variant}-seed{seed}-{run_index:03d}"
                )
                ...
                run_key = f"{variant}-seed{seed}-{run_index:03d}"
    ...
    for run_key, rows in by_run.items():
        ...
        for budget in time_budgets:
            ...
            budget_rows_raw.append(
                {
                    "run_key": run_key,
                    "variant": run_meta[run_key]["variant"],
                    "qa_sampler_mode": run_meta[run_key]["qa_sampler_mode"],
                    "subset_size": run_meta[run_key]["subset_size"],
    ...
    print(
        "Saved block-size comparison artifacts:\n"
